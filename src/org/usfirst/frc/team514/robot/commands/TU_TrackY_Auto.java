 ...
 * @author Team 514
 */
public class TU_TrackY_Auto extends Command {
    int myTarget;
    boolean status;
    
    public TU_TrackY_Auto(int target) {
...
//        oi.setTargetStatus(targetsUtil.onTargetY());
    if(myTarget == 3){    
       if((Robot.angleUtil.getPot() > (RobotMap.tu_3PT - RobotMap.sau_WINDOW_AUTO)) && 
           (Robot.angleUtil.getPot() < (RobotMap.tu_3PT + RobotMap.sau_WINDOW_AUTO))){
            status = true;
        }
    }else{
       if(Robot.targetsUtil.onTargetY()){
            status = true;
        }
    }
        return status;
